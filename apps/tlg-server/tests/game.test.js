...
describe("Game Services", () => {
    ...
    it("should tap in active round", async () => {
        ...
        const round = await broker.call("rounds.create", {
            start: new Date(),
        }, {
            meta: {
                token: Admin.token,
        ...
        const tapRes = await broker.call("taps.tap", {
            roundId: round.id,
        }, {
            meta: {
                token: Admin.token,
    ...
    it("should return 0 for nikita", async () => {
        ...
        expect(nikitaTaps.every(({ score }) => score === 0)).toStrictEqual(true);
        expect(nikitaTaps.map(({ taps }) => taps).sort()).toStrictEqual([
            1, 2, 3, 4,
        ]);
        await expect(broker.call("rounds.get", {
            id: round.id,
